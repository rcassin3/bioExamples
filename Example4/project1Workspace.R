...
# Package names
packages <- c("tidyverse", "jsonlite", "tcltk", "dplyr", "ggpubr", "psych", "mnormt")
...
#create CO2 Emissions dataframe and check trend
df <- select (dataCO2, c('Year', 'Total'))
windows()
ggplot( df, aes(x = Year,y = Total) ) + geom_line( color = "blue", linetype = "solid",  size = 1)

#create Sea Level Change dataframe and check trend, also format the date so that it plays nice with the CO2 emissions dates
